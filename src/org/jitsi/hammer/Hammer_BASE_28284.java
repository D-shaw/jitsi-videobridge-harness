...
public class Hammer {
	...
	/**
	 * The base of the nickname use by all the virtual users this Hammer will
	 * create.
	 */
	private final String nickname;

	/**
	 * The information about the XMPP server to which all virtual users will try
	 * to connect.
	 */
	 ...
	 * files of their <tt>BundleActivator</tt> implementations). An element of
	 * the <tt>BUNDLES</tt> array is an array of <tt>String</tt>s and represents
	 * an OSGi start level.
	 */
	 ...
	 *
	 * @param host
	 *            The information about the XMPP server to which all virtual
	 *            users will try to connect.
	 * @param mdc
	 * @param nickname
	 *            The base of the nickname used by all the virtual users.
	 * @param numberOfUser
	...
	public static void init() {
		/**
		 * This code is a slightly modified copy of the one found in startOSGi
		 * of the class ComponentImpl of jitsi-videobridge.
		 *
	 ...
	 * @param credentials
	 *            a list of <tt>Credentials</tt> used for the login of the fake
	 *            users.
	 * @param overallStats
	 *            enable or not the logging of the overall stats computed at the
	 *            end of the run.
	 * @param allStats
	 *            enable or not the logging of the all the stats collected by
	 *            the <tt>HammerStats</tt> during the run.
	 * @param summaryStats
	 *            enable or not the logging of the dummary stats computed from
	 *            all the streams' stats collected by the <tt>HammerStats</tt>
	 *            during the run.
	 * @param statsPollingTime
	 *            the number of seconds between two polling of stats by the
	 *            <tt>HammerStats</tt> run method.
	 */
	public void start(int wait, boolean disableStats, List<Credential> credentials, boolean overallStats,
			boolean allStats, boolean summaryStats, int statsPollingTime) {
		if (wait <= 0)
			wait = 1;
		if (started) {
	 ...
	 *            the <tt>HammerStats</tt> during the run.
	 * @param summaryStats
	 *            enable or not the logging of the summary stats computed from
	 *            all the streams' stats collected by the <tt>HammerStats</tt>
	 *            during the run.
	 * @param statsPollingTime
	 *            the number of seconds between two polling of stats by the
	...
	/**
	 * Stop the streams of all the fake users created, and disconnect them from
	 * the MUC and the XMPP server. Also stop the <tt>HammerStats</tt> thread.
	 */
	public void stop() {
		if (!this.started) {
		 ...
		 * hammerStatsThread, to allow it to cleanly stop.
		 */
