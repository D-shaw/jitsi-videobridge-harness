...
/**
 * <tt>FakeUser</tt> simulate a Jitsi Meet user by setting up a stream and then
 * sending fake audio/video data using RTP to the videobridge.
 */
public class FakeUser implements PacketListener {
	/**
	 * The <tt>Logger</tt> used by the <tt>FakeUser</tt> class and its instances
	 * for logging output.
	 */
	private static final Logger logger = Logger.getLogger(FakeUser.class);

	/**
	 * The XMPP server info to which this <tt>FakeUser</tt> will communicate
	 */
	...
	/**
	 * The nickname/nickname taken by this <tt>FakeUser</tt> in the MUC chatroom
	 */
	 ...
	 */
	Map<String, List<MediaFormat>> possibleFormatMap = new HashMap<String, List<MediaFormat>>();

	/**
	 * A Map mapping a media type (audio, video, data), with a
	 * <tt>MediaFormat</tt> representing the selected format for the stream
	 * handling this media type.
	 *
	 ...
	 * RTPExtension representing the selected RTP extensions for the format (and
	 * its corresponding <tt>MediaDevice</tt>)
	 */
	Map<String, List<RTPExtension>> selectedRtpExtension = new HashMap<String, List<RTPExtension>>();

	/**
	 * The IQ message received by the XMPP server to initiate the Jingle
	 * session.
	 *
	 * It contains a list of <tt>ContentPacketExtension</tt> representing the
	 * media and their formats the videobridge is offering to send/receive and
	 * their corresponding transport information (IP, port, etc...).
	 */
	private JingleIQ sessionInitiate;

	/**
	 * The IQ message send by this <tt>FakeUser</tt> to the XMPP server to
	 * accept the Jingle session.
	 *
	 * It contains a list of <tt>ContentPacketExtension</tt> representing the
	 * media and format, with their corresponding transport information, that
	 * this <tt>FakeUser</tt> accept to receive and send.
	 */
	...
	/**
	 * Instantiates a <tt>FakeUser</tt> with a default nickname that will
	 * connect to the XMPP server contained in <tt>hostInfo</tt>.
	 *
	 * @param hostInfo
	 *            the XMPP server information needed for the connection.
	 * @param mdc
	 *            The <tt>MediaDeviceChooser</tt> that will be used by this
	 *            <tt>FakeUser</tt> to choose the <tt>MediaDevice</tt> for each
	 *            of its <tt>MediaStream</tt>s.
	 */
	public FakeUser(HostInfo hostInfo, MediaDeviceChooser mdc) {
		this(hostInfo, mdc, null, true);
	}

	/**
	 * Instantiates a <tt>FakeUser</tt> with a specified <tt>nickname</tt> that
	 ...
	 *            of its <tt>MediaStream</tt>s.
	 * @param nickname
	 *            the nickname used by this <tt>FakeUser</tt> in the connection.
	 *
	 */
	public FakeUser(HostInfo hostInfo, MediaDeviceChooser mdc, String nickname, boolean statisticsEnabled) {
	 ...
	 *            the nickname used by this <tt>FakeUser</tt> in the connection.
	 * @param smackDebug
	 *            the boolean activating or not the debug screen of smack
	 */
	public FakeUser(HostInfo hostInfo, MediaDeviceChooser mdc, String nickname, boolean smackDebug,
		...
		config = new ConnectionConfiguration(serverInfo.getXMPPHostname(), serverInfo.getPort(),
		...
		/*
		 * Creation in advance of the MediaStream that will be used later so the
		 * HammerStats can register their MediaStreamStats now.
		 */
	 ...
	 *             if the connection to the XMPP server goes wrong
	 */
	public void start() throws XMPPException {
		//logger.info(this.nickname + " : Login anonymously to the XMPP server.");
		//System.out.println(this.nickname + " in FakeUser.start().");
		connection.connect();
	...
	public void start(String username, String password) throws XMPPException {
		//logger.info(this.nickname + " : Login with username " + username + " to the XMPP server.");
		connection.connect();
	 ...
	 * 
	 * @throws XMPPException
	 *             if the connection to the MUC goes wrong
	 */
	private void connectMUC() {
		String roomURL = serverInfo.getRoomName() + "@" + serverInfo.getMUCDomain();
		//System.out.println(this.nickname + " connect to MUC " + roomURL);
		...
		while (true) {
			try {
				...
				muc.sendMessage("Born to crash!");

				/*
				 * Send a Presence packet containing a Nick extension so that
				 * the nickname is correctly displayed in jitmeet
				 */
				...
				/*
				 * If the nickname is already taken in the MUC (code 409) then
				 * we append '_' to the nickname, and retry
				 */
				if ((e.getXMPPError() != null) && (e.getXMPPError().getCode() == 409)) {
					logger.warn(this.nickname + " nickname already used, " + "changing to " + nickname + '_');
					nickname = nickname + '_';
					continue;
				} else {
	...
	/**
	 * Stop and close all media stream and disconnect from the MUC and the XMPP
	 * server
	 */
	public void stop() {
		//logger.info(
		//		this.nickname + " : stopping the streams, leaving the MUC" + " and disconnecting from the XMPP server");
		if (agent != null)
			agent.free();

		/*
		 * for(MediaStream stream : mediaStreamMap.values()) { stream.stop();
		 * System.out.println("after stream stop!"); stream.close(); }
		 * if(connection !=null) { if(sessionAccept != null) {
		 * connection.sendPacket( JinglePacketFactory.createSessionTerminate(
		 * sessionAccept.getFrom(), sessionAccept.getTo(),
		 * sessionAccept.getSID(), Reason.GONE, "Bye Bye")); }
		 * 
		 * if(muc != null) muc.leave(); connection.disconnect(); }
		 */
	...
	/**
	 * acceptJingleSession create a accept-session Jingle message and send it to
	 * the initiator of the session. The initiator is taken from the From
	 * attribute of the initiate-session message.
	 */
	private void acceptJingleSession() {
		...
		for (ContentPacketExtension cpe : sessionInitiate.getContentList()) {
			...
			if (cpe.getName().equalsIgnoreCase("data")) {
				...
				description = cpe.getFirstChildOfType(RtpDescriptionPacketExtension.class);

				if (description == null)
					continue;

				listFormat = JingleUtils.extractFormats(description, ptRegistry);
				remoteRtpExtension = JingleUtils.extractRTPExtensions(description, rtpExtRegistry);
				supportedRtpExtension = getExtensionsForType(MediaType.parseString(cpe.getName()));
				listRtpExtension = intersectRTPExtensions(remoteRtpExtension, supportedRtpExtension);
				...
				selectedRtpExtension.put(cpe.getName(), listRtpExtension);

				content = JingleUtils.createDescription(CreatorEnum.responder, cpe.getName(), SendersEnum.both,
						listFormat, listRtpExtension, ptRegistry, rtpExtRegistry);
			}

			contentMap.put(cpe.getName(), content);
		}
		/*
		 * We remove the content for the data (because data is not handle for
		 * now by libjitsi) FIXME
		 */
		...
		try {
			...
			logger.fatal(this.nickname + " : Error during the generation" + " of the IceMediaStream", e);
		}
		 ...
		 * MediaDeviceChooser.
		 */
		HammerUtils.configureMediaStream(mediaStreamMap, selectedFormat, selectedRtpExtension, mediaDeviceChooser,
				ptRegistry, rtpExtRegistry);
		...
		/*
		 * Send the SSRC of the different media in a "media" tag It's not
		 * necessary but its a copy of Jitsi Meet behavior
		 *
		 * Also, without sending this packet, there are error logged in the
		 * javascript console of the Jitsi Meet initiator :
		 * "No video type for ssrc: 13365845" It seems like Jitsi Meet can work
		 * arround this error, but better safe than sorry.
		 */
		presencePacketWithSSRC = new Presence(Presence.Type.available);
		String recipient = serverInfo.getRoomName() + "@" + serverInfo.getMUCDomain() + "/" + nickname;
		...
		sessionAccept = JinglePacketFactory.createSessionAccept(sessionInitiate.getTo(), sessionInitiate.getFrom(),
				sessionInitiate.getSID(), contentMap.values());
		...
		HammerUtils.setDtlsEncryptionOnTransport(mediaStreamMap, sessionAccept.getContentList(),
		...
		PropertyChangeListener propertyChangeListener = new PropertyChangeListener() {
			...
			public void propertyChange(PropertyChangeEvent ev) {
				...
				if (IceProcessingState.COMPLETED.equals(newValue) || IceProcessingState.FAILED.equals(newValue)
		...
		IceProcessingState iceState = agent.getState();
		if (!IceProcessingState.COMPLETED.equals(iceState) && !IceProcessingState.TERMINATED.equals(iceState)) {
			logger.fatal("ICE failed for user " + nickname + ". Agent state: " + iceState);
			return;
		}
	 ...
	 * 
	 * @param packet
	 *            the packet received by the <tt>FakeUser</tt>
	 */
	public void processPacket(Packet packet) {
		...
		case SESSION_INITIATE:
			...
			if (sessionInitiate == null) {
				...
				// TODO FIXME It need to be changed if Jitsi-Hammer want to be
				// used with Jitsi
	 ...
	 * supported by the device that this <tt>FakeUser</tt> uses to handle media
	 * of the specified <tt>type</tt>.
	 *
	 * @param type
	 *            the <tt>MediaType</tt> of the device whose
	 ...
	 * Copy from CallPeerMediaHandler class of Jitsi
	 *
	 * Compares a list of <tt>RTPExtension</tt>s offered by a remote party to
	 * the list of locally supported <tt>RTPExtension</tt>s as returned by one
	 * of our local <tt>MediaDevice</tt>s and returns a third <tt>List</tt> that
	 * contains their intersection. The returned <tt>List</tt> contains
	 * extensions supported by both the remote party and the local device that
	 * we are dealing with. Direction attributes of both lists are also
	 * intersected and the returned <tt>RTPExtension</tt>s have directions valid
	 * from a local perspective. In other words, if <tt>remoteExtensions</tt>
	 * contains an extension that the remote party supports in a
	 * <tt>SENDONLY</tt> mode, and we support that extension in a
	 ...
	 *            the <tt>List</tt> of <tt>RTPExtension</tt>s as advertised by
	 *            the remote party.
	 * @param supportedExtensions
	 *            the <tt>List</tt> of <tt>RTPExtension</tt>s that a local
	 *            <tt>MediaDevice</tt> returned as supported.
	 *
	 * @return the (possibly empty) intersection of both of the extensions lists
	 *         in a form that can be used for generating an SDP media
	 *         description or for configuring a stream.
	 */
	 ...
	 * Copy from CallPeerMediaHandler class of Jitsi
	 *
	 * Returns the first <tt>RTPExtension</tt> in <tt>extList</tt> that uses the
	 * specified <tt>extensionURN</tt> or <tt>null</tt> if <tt>extList</tt> did
	 * not contain such an extension.
	 *
	 * @param extList
	 ...
	 *
	 * @return the first <tt>RTPExtension</tt> in <tt>extList</tt> that uses the
	 *         specified <tt>extensionURN</tt> or <tt>null</tt> if
	 *         <tt>extList</tt> did not contain such an extension.
	 */
